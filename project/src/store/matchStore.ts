...
export const useMatchStore = create<MatchStore>((set, get) => ({
  ...
  joinMatch: async (matchId: string, itemIds: string[]) => {
    ...
    // Update match with member_id and combined items_ids using a transaction-like approach
    const allItems = [...matchData.items_ids, ...itemIds];
    
    console.log('Attempting to update match with:', {
      member_id: user.id,
      items_ids: allItems,
      matchId: matchId
    });
