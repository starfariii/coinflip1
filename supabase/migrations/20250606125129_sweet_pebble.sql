/*
  # Fix match completion rewards system

  1. Changes
    - Drop existing trigger and function in correct order
    - Recreate handle_match_completion function with proper winner logic
    - Recreate trigger to call the function

  2. Security
    - Function runs with SECURITY DEFINER to bypass RLS
    - Proper winner determination based on coin flip result
    - All match items go to the winner
*/

-- Drop the trigger first (this removes the dependency)
DROP TRIGGER IF EXISTS on_match_completion ON matches;

-- Now we can safely drop the function
DROP FUNCTION IF EXISTS handle_match_completion();

-- Recreate the function with proper winner logic
